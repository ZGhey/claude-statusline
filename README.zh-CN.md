...
脚本就用它作为判定信号,它同时驱动两个互补的字段:

- **订阅用户**(存在 `rate_limits`)→ 成本**隐藏**、限额组**显示**
- **API / console 计费**(无 `rate_limits`)→ 成本**显示**(那是你真实的账单)、限额组**隐藏**(按 token 计费没有限流窗口)
...
> **在用 usage credits?** 订阅用户额度用完、开启了按量付费的 credits 后,这笔花费
> 是**真实的** —— 但状态栏 JSON 没有任何字段能识别这个状态(所以脚本无法自动切换,
> 否则对更常见的"被限流、只是干等重置"的情况会重新显示出幻象成本)。如果你在用
> credits 且想盯着花费,设置 `STATUSLINE_SHOW_COST=1` —— 成本就会和限额组及其重置
> 时间一起显示出来。
